...
def download_remote_file(
    ...
    with http.get(
        ...
        # FIX: Éviter la race condition avec une boucle sûre
        if output.exists():
            stem = output.stem
...
def multiup_login() -> str:
    ...
    # FIX: Utiliser is_success_response pour la cohérence
    if not is_success_response(data):
        error_msg = data.get("error", "Erreur inconnue")
        raise UploadError(f"Multiup login : {error_msg}")
...
def upload_to_providers(
    ...
    # FIX: Ajouter remote_name en paramètre et passer None aux uploaders
    results = []
    ...
    for provider in providers:
        ...
        try:
            result = UPLOADERS[provider](path, remote_name)
    ...
    for index, url in enumerate(urls):
        ...
        results.extend(
            upload_to_providers(
                file_path,
                providers,
                # FIX: Passer le nom custom si disponible
...
def run_archive(
    ...
    return upload_to_providers(
        archive_path,
        providers,
        # FIX: Passer le nom de l'archive
        ...
        for file_path in extracted_files:
            results.extend(
                upload_to_providers(
                    file_path,
                    providers,
                    # FIX: Utiliser None pour laisser le nom par défaut
                    remote_name=None,
                )
            )
